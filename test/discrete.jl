...
@testset "Example 5.4" begin
    ...
    qub = 4.3195961
    @test isapprox(quicklb(s), 3)
    @test isapprox(quickub(s), qub)
    qb = quickb(s)
    @test typeof(qb) == NTuple{2,Float64}
    @test isapprox(qb[1], 3)
    @test isapprox(qb[2], qub)
    ...
    @test isapprox(pradius(s, 2), 3.234535151)
    @test isapprox(pradius(s, 2, :BruteForce, pnorm=2), 3.632097274822649)
    lb, ub = pradiusb(s, 2)
    @test isapprox(lb, 3.234535151)
    @test isapprox(ub, 4.574323479)
    lb, ub = pradiusb(s, 4)
    @test isapprox(lb, 3.427560156)
    @test isapprox(ub, 4.076078925)
    smp = DiscretePeriodicSwitching(s, [1, 2])
    for solver in sdp_solvers
        try println("  > With solver $(typeof(solver))")
            lb, ub = soslyapb(s, 1, solver=solver)
            @test isapprox(lb, 2.814640557, rtol=1e-5)
            @test isapprox(ub, 3.980502849, rtol=1e-5)
            lb, ub = soslyapb(s, 2, solver=solver)
            @test isapprox(lb, 3.299750624, rtol=1e-5)
            @test isapprox(ub, 3.924086919, rtol=1e-5)
    ...
    if isempty(sdp_solvers)
        @test isapprox(s.ub, 4.076078925)
    else
        @test isapprox(s.ub, 3.924086919, rtol=1e-5)
    end
end
