...
abstract type AbstractSOSData{S} end
mutable struct SOSData{S} <: AbstractSOSData{S}
    ...
    upsw::Union{Nothing, Set{DiscretePeriodicSwitching{S}}}
end
function newsosdata(s::DiscreteSwitchedLinearSystem)
    n = statedim(s, 1)
    @polyvar x[1:n]
    SOSData{typeof(s)}(x, 0, Inf, Union{Nothing, Lyapunov}[], nothing, nothing)
end

import LightGraphs
mutable struct ConstrainedSOSData{S} <: AbstractSOSData{S}
...
function newsosdata(s::ConstrainedDiscreteSwitchedLinearSystem)
    ...
    ConstrainedSOSData{typeof(s)}(y, eid, 0, Inf, Union{Nothing, Vector{Lyapunov}}[], nothing, nothing)
end
...
function compute_unstable_periodic_switchings!(s::AbstractSOSData{S}) where S
    s.upsw = Set{DiscretePeriodicSwitching{S}}()
end
