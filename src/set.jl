...
ConeLyap(p::P, Q::Matrix{S}, b::Vector{S}, β::S, c, H) where {T, P<:AbstractPolynomial{T}, S} = ConeLyap{T, P, S}(p, Q, b, β, c, H)
JuMP.getvalue(p::ConeLyap) = ConeLyap(getvalue(p.p), getvalue(p.Q), getvalue(p.b), getvalue(p.β), p.c, p.H)
